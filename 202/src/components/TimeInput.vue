  ...
  <div class="time-change-model">
    <div class="time-change-div">
      ...
      <div class="date-model">
        ...
        <!-- <div><input type="date" style="margin-right: 5px;" class="date" value="2017/12/01"/></div> -->
        <div><input type="number" style="margin-right: 5px;" v-model="month" @blur="check('month')" />:<input
            type="number" style="margin-left: 5px;margin-right: 5px;" v-model="day" @blur="check('day')" />:<input
            type="number" style="margin-left:5px;" v-model="year" @blur="check('year')" /></div>
      </div>
      <div class="date-model" style="margin-top: 31px;margin-bottom: 40px;">
        <div class="date-text" style="margin-right: 13px;">{{$t("language.timeInput")}}</div>
        <div><input type="number" style="margin-right: 5px;" v-model="hour" min='1' max='23'
            @blur="check('hour')" />:<input type="number" style="margin-left: 5px;margin-right: 5px;" v-model="minute"
            @blur="check('minute')" />:<input type="number" style="margin-left:5px;" v-model="seconds"
            @blur="check('seconds')" /></div>
      </div>
      <div class="buttons">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changeTimeOk() {
        if (this.year == null || this.month == null || this.day == null || this.hour == null || this.minute == null ||
          this.seconds == null) {
          ...
          let timeString = '' + this.year + '-' + this.month + '-' + this.day + ' ' + this.hour + ':' + this
            .minute + ':' + this.seconds;
          let time_stamp = Number(new Date(timeString));
          updateSystem({
            'system_time': time_stamp.toString().substring(0, time_stamp.toString().length - 3)
          }).then((res) => {
