<template>
  <div class="model" v-if="title_type==1">
    <div class="step-name">
      {{$t("language.step")}}:
      <input class="input-step-name" v-model="discard.step_name" @blur="saveData()" />
    </div>
    ...
    <div class="step-name">
      ...
      <input class="input-step-name" v-model="discard.volume" @blur="saveData()" />
    </div>
    <div class="step-name">
      {{$t("language.mix_time")}}:
      <input class="input-step-name" v-model="discard.mix_time" @blur="saveData()" />
    </div>
    <div class="step-name">
      {{$t("language.mix_speed")}}:
      <input class="input-step-name" v-model="discard.mix_speed" @blur="saveData()" />
    </div>
    <div class="step-name">
      {{$t("language.mix_height")}}:
      <input class="input-step-name" v-model="discard.mix_height" @blur="saveData()" />
    </div>
    <div class="step-name">
      {{$t("language.mix_volume")}}:
      <input class="input-step-name" v-model="discard.mix_volume" @blur="saveData()" />
    </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getData() {
        this.discard = this.$store.getters.protocol.steps[this.order - 1]
      },
      saveData() {
        var protocol = JSON.parse(JSON.stringify(this.$store.getters.protocol))
        protocol.steps[this.order - 1] = this.discard
        this.$store.commit('setProtocol', protocol)
