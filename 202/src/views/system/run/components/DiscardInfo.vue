<template>
  <div :class="[whitchStyle==='view'?'info-view' : 'info-run']">
    ...
    <div> <span>{{$t('language.well')}}(1-C):</span>  <span>{{info.well}}</span></div>
    <div> <span>{{$t('language.volume')}}(5-1000μl):</span>  <span>{{info.volume}}</span></div>
    <div> <span>{{$t('language.mix_time')}}(1-99min):</span>  <span>{{info.mix_time}}</span></div>
    <div> <span>{{$t('language.mix_speed')}}(1-10):</span>  <span>{{info.mix_speed}}</span></div>
    <div> <span>{{$t('language.mix_height')}}(0-50mm):</span>  <span>{{info.mix_height}}</span></div>
    <div> <span>{{$t('language.mix_volume')}}(5-1000μl):</span>  <span>{{info.mix_volume}}</span></div>
  </div>
</template>
