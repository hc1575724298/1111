<template>
  <div :class="[whitchStyle==='view'?'info-view' : 'info-run']">
    ...
    <div> <span>{{$t('language.well')}}(1 / A):</span> <span>{{info.well}}</span></div>
    <div> <span>{{$t('language.temperature')}}(10-110℃):</span> <span>{{info.incubator_temperature}}</span></div>
    <div> <span>{{$t('language.time')}}(0-999min):</span> <span>{{info.incubator_time}}</span></div>
  </div>
</template>
